...
exports.enrollStudent = async (req, res) => {
    try {
        const { studentId, courseId } = req.body;
        if (!studentId || !courseId) return res.status(400).json({ message: "studentId & courseId required" });
        const s = await Student.findById(studentId); if (!s) return res.status(404).json({ message: "Student not found" });
        const c = await Course.findById(courseId); if (!c) return res.status(404).json({ message: "Course not found" });
        const enroll = await Enrollment.create({ student: studentId, course: courseId });
